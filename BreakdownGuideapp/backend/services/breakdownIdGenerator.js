  ...
  async getNextNumberFromDatabase(year) {
    try {
      ...
      const { data: latestBreakdown, error: latestError } = await supabase
        .from('breakdowns')
        .select('breakdown_id')
        .like('breakdown_id', `BD-${year}-%`)
        .order('breakdown_id', { ascending: false })
        .limit(1);
      
      if (!latestError && latestBreakdown && latestBreakdown.length > 0) {
        const lastId = latestBreakdown[0].breakdown_id;
  ...
  async getStatistics(year = null) {
    ...
    try {
      ...
      const { data: monthlyData, error: monthlyError } = await supabase
        .from('breakdowns')
        .select('created_at')
        .gte('created_at', `${targetYear}-${currentMonth.toString().padStart(2, '0')}-01T00:00:00.000Z`)
        .lt('created_at', `${targetYear}-${(currentMonth + 1).toString().padStart(2, '0')}-01T00:00:00.000Z`);
      ...
      return {
        year: targetYear,
        total_breakdowns: totalCount || 0,
        next_sequence: (totalCount || 0) + 1,
