-- Migration: Add Engineering Tracking Columns to Breakdowns Table
-- Purpose: Track engineer job lifecycle from assignment to completion
-- Created: October 4, 2025

-- Add engineering workflow timestamps
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_accepted_at TIMESTAMPTZ;
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_on_site_at TIMESTAMPTZ;
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_fixing_at TIMESTAMPTZ;
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_completed_at TIMESTAMPTZ;

-- Add engineering data fields
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_notes JSONB DEFAULT '[]'::jsonb;
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS parts_used JSONB;
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS labor_hours DECIMAL(5,2);
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS repair_category VARCHAR(100);
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS root_cause TEXT;

-- Add engineer assignment tracking
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_id VARCHAR(50);
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_name VARCHAR(100);
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_badge VARCHAR(20);
ALTER TABLE breakdowns ADD COLUMN IF NOT EXISTS engineer_eta_minutes INTEGER;
...
-- Insert sample engineers for testing (optional - comment out in production)
INSERT INTO engineers (badge_number, name, email, depot, skills, status) VALUES
  ('ENG001', 'John Smith', 'john.smith@gonortheast.co.uk', 'Washington', '["electrical", "mechanical"]', 'available'),
  ('ENG002', 'Sarah Johnson', 'sarah.johnson@gonortheast.co.uk', 'Riverside', '["hvac", "mechanical"]', 'available'),
  ('ENG003', 'Mike Williams', 'mike.williams@gonortheast.co.uk', 'Consett', '["electrical", "diagnostics"]', 'available'),
  ('ENG004', 'Emma Brown', 'emma.brown@gonortheast.co.uk', 'Washington', '["mechanical", "hydraulics"]', 'available'),
  ('ENG005', 'David Wilson', 'david.wilson@gonortheast.co.uk', 'Deptford', '["electrical", "mechanical", "hvac"]', 'available')
ON CONFLICT (badge_number) DO NOTHING;

-- Add RLS policies for engineers table (if using Row Level Security)
-- ALTER TABLE engineers ENABLE ROW LEVEL SECURITY;

-- CREATE POLICY "Engineers are viewable by authenticated users" ON engineers
--   FOR SELECT USING (auth.role() = 'authenticated');

-- CREATE POLICY "Engineers can update their own status" ON engineers
