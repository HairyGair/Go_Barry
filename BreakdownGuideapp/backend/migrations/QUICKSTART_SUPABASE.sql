-- QUICKSTART: Engineering Dashboard Setup for Supabase
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_accepted_at') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_accepted_at TIMESTAMPTZ;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_on_site_at') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_on_site_at TIMESTAMPTZ;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_fixing_at') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_fixing_at TIMESTAMPTZ;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_completed_at') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_completed_at TIMESTAMPTZ;
    END IF;

    -- Add engineering data fields
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_notes') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_notes JSONB DEFAULT '[]'::jsonb;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='parts_used') THEN
        ALTER TABLE breakdowns ADD COLUMN parts_used JSONB;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='labor_hours') THEN
        ALTER TABLE breakdowns ADD COLUMN labor_hours DECIMAL(5,2);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='repair_category') THEN
        ALTER TABLE breakdowns ADD COLUMN repair_category VARCHAR(100);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='root_cause') THEN
        ALTER TABLE breakdowns ADD COLUMN root_cause TEXT;
    END IF;

    -- Add engineer assignment tracking
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_id') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_id VARCHAR(50);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_name') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_name VARCHAR(100);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_badge') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_badge VARCHAR(20);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='breakdowns' AND column_name='engineer_eta_minutes') THEN
        ALTER TABLE breakdowns ADD COLUMN engineer_eta_minutes INTEGER;
    END IF;

    RAISE NOTICE '✅ Breakdowns table columns added successfully';
END $$;
