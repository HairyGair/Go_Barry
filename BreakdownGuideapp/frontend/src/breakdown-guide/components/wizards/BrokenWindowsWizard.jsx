...
const BrokenWindowsWizard = ({ currentStep, responses, updateResponse, onNext, onPrevious, onComplete }) => {
    ...
    const handleCompleteAssessment = () => {
        ...
        if (mustStop) {
            ...
            if (responses.driver_fit === 'no') {
                notes += 'Driver injured or unfit to continue. ';
            }
            if (responses.passengers_unharmed === 'no') {
                notes += 'Passenger injuries reported. ';
            }
            if (responses.vehicle_unsafe === 'yes') {
                notes += 'Driver view impaired or danger to occupants present. ';
            }
            if (responses.damage_extent === 'critical_systems') {
                notes += 'Critical vehicle systems affected by vandalism. ';
            }
            if (responses.engineering_needed === 'yes') {
                notes += 'Engineering assessment required. ';
            }
            
            notes += 'Medical attention and/or engineering assistance required immediately.';
        } else if (canContinueWithVigilance) {
            finalDecision = 'AMBER';
        ...
        <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-8">
            <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-400">Broken Windows Assessment</span>
                </div>
...
export default BrokenWindowsWizard;