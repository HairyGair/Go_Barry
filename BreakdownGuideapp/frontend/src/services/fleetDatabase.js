...
// Now connects to production Supabase via API

import { fleetAPI } from './api-client.js';
...
class FleetDatabaseService {
    ...
    async loadFleetDatabase() {
        try {
            if (this.enableMockData) {
                console.log('🔄 Loading fleet database from local JSON (mock data mode)...');
                await this.loadLocalFleetData();
            } else {
                console.log('🔄 Loading fleet database from production Supabase...');
                await this.loadProductionFleetData();
            }
        } catch (error) {
            console.error('❌ Failed to load fleet database:', error);
            // Fallback to local data if production fails
            if (!this.enableMockData) {
                console.log('🔄 Falling back to local fleet data...');
                await this.loadLocalFleetData();
            }
        }
    }

    async loadProductionFleetData() {
        try {
            const response = await fleetAPI.getAll({ limit: 1000 });
            
            if (response?.data) {
                this.fleetData = this.transformProductionFleetData(response.data);
                console.log('✅ Production fleet database loaded:', Object.keys(this.fleetData).length, 'vehicles');
            } else {
                throw new Error('No fleet data received from production API');
            }
        } catch (error) {
            console.error('❌ Failed to load production fleet data:', error);
            throw error;
        }
    }
    ...
    async loadLocalFleetData() {
        try {
            ...
            console.log('📥 Local fleet data loaded:', rawData.totalVehicles, 'vehicles');
            
            this.fleetData = this.transformRealFleetData(rawData);
            console.log('✅ Local fleet database loaded:', Object.keys(this.fleetData).length, 'vehicles');
        } catch (error) {
            console.error('❌ Failed to load local fleet data:', error);
            this.fleetData = {};
        }
    }
    ...
    // Transform production fleet data from Supabase
    transformProductionFleetData(fleetData) {
        const transformedData = {};
        ...
        fleetData.forEach(vehicle => {
            ...
            transformedData[fleetNumber] = {
                ...
                // Additional fields from production
                capacity: vehicle.capacity,
                yearOfManufacture: vehicle.year_of_manufacture,
                euroRating: vehicle.euro_rating,
                engineType: vehicle.engine_type,
                vehicleTypeCategory: vehicle.category || 'Bus',
                age: vehicle.year_of_manufacture ? new Date().getFullYear() - vehicle.year_of_manufacture : null,
                // Supabase metadata
                id: vehicle.id,
        ...
        console.log(`🚌 Loaded ${Object.keys(transformedData).length} vehicles from production Supabase`);
