...
### Production
```
Primary:    https://go-barry.onrender.com
...
### WebSocket URLs
```
Production: wss://go-barry.onrender.com/ws
...
curl -X GET https://go-barry.onrender.com/api/breakdowns/live \
...
```bash
curl -X GET https://go-barry.onrender.com/api/auth/validate \
...
```bash
curl -X POST https://go-barry.onrender.com/api/auth/logout \
  -H "Authorization: Bearer YOUR_TOKEN_HERE"
```
...
```bash
curl https://go-barry.onrender.com/api/auth/supervisors
```
...
```bash
curl -X POST https://go-barry.onrender.com/api/breakdowns \
  -H "Content-Type: application/json" \
  -d '{
    "fleet_no": "6127",
...
```bash
curl "https://go-barry.onrender.com/api/fleet?search=6127&depot=Washington"
```

**Example: Get vehicle details**
```bash
curl https://go-barry.onrender.com/api/fleet/vehicle/6127
```
...
```bash
curl "https://go-barry.onrender.com/api/activity/feed?limit=50&depot=Washington"
```

**Example: Log activity**
```bash
curl -X POST https://go-barry.onrender.com/api/activity/log \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://go-barry.onrender.com/api/engineering/jobs
```

**Example: Assign engineer**
```bash
curl -X POST https://go-barry.onrender.com/api/engineering/assign \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://go-barry.onrender.com/api/defects/repeat \
  -H "Content-Type: application/json" \
  -d '{
    "timeframe": "7d",
    "min_occurrences": 3
...
**Example: Get KPIs**
```bash
curl "https://go-barry.onrender.com/api/analytics/kpis?period=week"
```
...
```bash
curl https://go-barry.onrender.com/api/public/breakdowns/live
```
...
**Total Channels:** 5
**Connection URL:** `wss://go-barry.onrender.com/ws?channel=CHANNEL_NAME&token=JWT_TOKEN`
...
const ws = new WebSocket(
  'wss://go-barry.onrender.com/ws?channel=sdc-dashboard&token=YOUR_JWT_TOKEN'
);
...
**Connection Example:**
```javascript
const ws = new WebSocket('wss://go-barry.onrender.com/ws?channel=control-room');
...
**Connection Example:**
```javascript
const ws = new WebSocket('wss://go-barry.onrender.com/ws?channel=defect-intelligence');
...
# =============================================================================
# Backend API URL
API_BASE_URL=https://go-barry.onrender.com/api

# Frontend App URL
APP_URL=https://go-barry.onrender.com

# CORS Origins (comma-separated)
ALLOWED_ORIGINS=http://localhost:5173,http://localhost:3000,https://go-barry.onrender.com,https://gobarry.co.uk
...
```bash
# 1. Login
TOKEN=$(curl -s -X POST https://go-barry.onrender.com/api/auth/login \
  -H "Content-Type: application/json" \
...
# 3. Validate token
curl -H "Authorization: Bearer $TOKEN" \
  https://go-barry.onrender.com/api/auth/validate

# 4. Logout
curl -X POST -H "Authorization: Bearer $TOKEN" \
  https://go-barry.onrender.com/api/auth/logout
```
...
curl -X POST https://go-barry.onrender.com/api/breakdowns/BD-2025-001/acknowledge \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST https://go-barry.onrender.com/api/breakdowns/BD-2025-001/request-engineering \
  -H "Content-Type: application/json" \
  -d '{
    "priority": "high",
    "eta_minutes": 20,
...
# 4. Resolve breakdown
curl -X POST https://go-barry.onrender.com/api/breakdowns/BD-2025-001/resolve \
  -H "Content-Type: application/json" \
  -d '{
    "resolved_by": "ENG001",
...
curl "https://go-barry.onrender.com/api/fleet?search=6127"

# Get vehicle details
curl https://go-barry.onrender.com/api/fleet/vehicle/6127

# Get all Washington depot vehicles
curl "https://go-barry.onrender.com/api/fleet?depot=Washington&limit=100"

# Get vehicle breakdown history
curl https://go-barry.onrender.com/api/breakdowns/fleet/6127

# Get fleet statistics
curl https://go-barry.onrender.com/api/fleet/stats/summary
```
...
# Get activity feed
curl "https://go-barry.onrender.com/api/activity/feed?limit=50"

# Get recent activity (last 5 minutes)
curl "https://go-barry.onrender.com/api/activity/live?since=2025-10-27T12:00:00Z"

# Get breakdown counters
curl https://go-barry.onrender.com/api/breakdowns/breakdown-counters
```
...
curl https://go-barry.onrender.com/api/analytics/depot-comparison

# Get breakdown trends
curl "https://go-barry.onrender.com/api/analytics/breakdown-trends?period=month"

# Get supervisor performance stats
curl https://go-barry.onrender.com/api/supervisors/AG003/stats?period=week
...
# Get job details
curl https://go-barry.onrender.com/api/engineering/job/BD-2025-001

# Accept job
curl -X POST https://go-barry.onrender.com/api/engineering/accept-job \
  -H "Content-Type: application/json" \
  -d '{
    "breakdown_id": "BD-2025-001",
    "engineer_id": "ENG001",
...
# Complete job
curl -X POST https://go-barry.onrender.com/api/engineering/complete-job \
  -H "Content-Type: application/json" \
  -d '{
    "breakdown_id": "BD-2025-001",
    "engineer_id": "ENG001",
...
const ws = new WebSocket('wss://go-barry.onrender.com/ws?channel=sdc-dashboard&token=TOKEN');
...
1. **Verify origin in ALLOWED_ORIGINS:**
   ```bash
   ALLOWED_ORIGINS=http://localhost:5173,https://go-barry.onrender.com
   ```
...
**WebSocket Channels:**
```
wss://go-barry.onrender.com/ws?channel=sdc-dashboard&token=JWT      (protected)
wss://go-barry.onrender.com/ws?channel=control-room                 (public)
wss://go-barry.onrender.com/ws?channel=defect-intelligence          (public)
```
