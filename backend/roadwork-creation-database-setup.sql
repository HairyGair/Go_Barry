...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conname = 'roadworks_status_check' 
        AND conrelid = 'roadworks'::regclass
    ) THEN
        ALTER TABLE roadworks ADD CONSTRAINT roadworks_status_check 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'roadworks_severity_check' 
        AND conrelid = 'roadworks'::regclass
    ) THEN
        ALTER TABLE roadworks ADD CONSTRAINT roadworks_severity_check 
