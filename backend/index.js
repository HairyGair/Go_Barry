...
app.get('/api/traffic-incidents', async (req, res) => {
  try {
    console.log('🚨 Direct traffic incidents endpoint called');
    // Import services
    ...
    const trafficIncidents = (intelligence.incidents || []).map(alert => {
      ...
      return {
        ...
        title: alert.title || `${alert.type || 'Traffic Alert'} - ${alert.location}`,
        description: alert.description || `Traffic ${alert.type || 'alert'} detected`,
...
(async () => {
  try {
    ...
    // Start automatic incident creator monitoring
    try {
      await autoIncidentCreator.startMonitoring();
      console.log('✅ Auto-incident creator monitoring started');
    } catch (error) {
      console.warn('⚠️ Auto-incident creator failed to start:', error.message);
    }
  } catch (error) {
    console.error('⚠️ Initialization error:', error.message);
    console.log('⚠️ Continuing with limited functionality...');
...
process.on('SIGTERM', () => {
  ...
  // Stop auto-incident creator monitoring
  try {
