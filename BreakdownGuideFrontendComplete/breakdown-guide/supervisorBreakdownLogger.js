...
(function() {
    ...
    class SupervisorBreakdownLogger {
        ...
        async startAssessment(wizardType, fleetNumber, depot) {
            ...
            // Call new API to start breakdown with CORS error handling
            try {
                ...
                if (data.success) {
                    ...
                    console.log('✅ Breakdown tracking started:', data.breakdown_id);
                    ...
                    throw new Error(data.error || 'Failed to start breakdown tracking');
                }
            } catch (error) {
                if (error.name === 'TypeError' && (error.message.includes('CORS') || error.message.includes('Failed to fetch'))) {
                    console.log('⚠️ CORS Error: Cannot connect to breakdown tracker from this origin');
                    console.log('📝 Operating in offline/mock mode');
                } else {
                    console.log('⚠️ Error starting breakdown tracking:', error.message);
                    console.log('📝 Operating in offline/mock mode');
                }
                
                // Continue with local tracking even if backend fails
                this.breakdownId = 'local_' + Date.now();
                this.dailyId = 'LOCAL-' + new Date().toISOString().slice(0, 10);
            }
        ...
        async logWizardStep(stepType, stepData) {
            ...
            try {
                const response = await fetch(`${BACKEND_URL}/api/breakdowns/step`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (response.ok) {
                    console.log('✅ Wizard step logged to tracker:', stepType);
                } else {
                    throw new Error(`HTTP ${response.status}`);
                }
            } catch (error) {
                if (error.name === 'TypeError' && (error.message.includes('CORS') || error.message.includes('Failed to fetch'))) {
                    console.log('⚠️ CORS Error: Cannot log step to tracker, logging locally instead');
                } else {
                    console.log('⚠️ Error logging step to tracker:', error.message);
                }
            }
        ...
        async completeWizardDiagnosis(severity, resolution) {
            ...
            try {
                ...
                if (response.ok) {
                    console.log('✅ Diagnosis completed in tracker:', severity);
                ...
                if (error.name === 'TypeError' && (error.message.includes('CORS') || error.message.includes('Failed to fetch'))) {
                    console.log('⚠️ CORS Error: Cannot complete diagnosis in tracker, showing Passenger Cloud anyway');
                } else {
                    console.log('⚠️ Error completing diagnosis in tracker:', error.message);
                }
                
                // Show Passenger Cloud option anyway, even if backend call failed
                this.showPassengerCloudOption();
            }
        }
        ...
        // Clear supervisor (called on logout)
        clearSupervisor() {
            this.supervisor = null;
            this.currentAssessment = null;
            this.assessmentStartTime = null;
            ...
            console.log('Supervisor session cleared from breakdown logger');
        }
    }
